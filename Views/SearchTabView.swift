...
struct SearchTabView: View {
    ...
    private var overlayView: some View {
        ...
        case .empty:
            if !searchVM.searchQuery.isEmpty {
                ...
                EmptyPlaceholderView(text: "Поиск чего-то нового", image: Image(systemName: "magnifyingglass"))
            }
            
        case .success(let articles) where articles.isEmpty:
            EmptyPlaceholderView(text: "Не найдено", image: Image(systemName: "magnifyingglass"))
    ...
    private var suggestionsView: some View {
        ForEach(["Спец операция", "ковид", "оспа", "лето", "рубль"], id: \.self) { text in
            Button {
